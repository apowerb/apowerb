...
``th2agent.scheduler.backlog_worker``, which drains the queue one row
...
from th2agent.storage.webhook_attachments import (
    resolve_attachment_path,
    store_webhook_attachment,
)
from th2agent.integrations.outlook_webhook import OutlookWebhookService
from th2agent.models import WebhookLog, WebhookSubscription
from th2agent.schema.webhook_schema import MicrosoftGraphNotificationPayload
...
async def handle_outlook_notification(
    ...
    **Public endpoint** — Microsoft Graph posts directly without our
    ...
    2. **Notification delivery** — verify ``clientState``, then INSERT a
       ...
       :func:`th2agent.scheduler.backlog_worker.process_once`, which
