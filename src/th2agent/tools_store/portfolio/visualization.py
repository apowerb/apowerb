...
def tool_visualize_data(
    ...
    if query and query.strip():
        try:
            from th2agent.tools_store.portfolio.text_to_sql import _agent_execute_sql
...
def make_visualization_tools(folder_name: str) -> list:
    ...
    Usage:
        from th2agent.tools_store.portfolio.visualization import make_visualization_tools
        tools_funcs.extend(make_visualization_tools("agent280"))
    """
    if not folder_name:
        raise ValueError("folder_name is required")

    import th2agent.tools_store.portfolio.visualization as _viz_module
