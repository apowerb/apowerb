...
from th2agent.core.agent_helpers.tools_binder import dedupe_tools_by_name
...
def test_keeps_first_occurrence():
    first = _named("tool_read_file", "th2agent.tools_store.portfolio.basic")
    second = _named("tool_read_file", "th2agent.tools_store.portfolio.onedrive_read")
...
def test_logs_warning_with_source_module(caplog):
    caplog.set_level(logging.WARNING, logger="th2agent.core.agent_helpers.tools_binder")
    first = _named("tool_read_file", "th2agent.tools_store.portfolio.basic")
    dup = _named("tool_read_file", "th2agent.tools_store.portfolio.onedrive_read")
...
def test_preserves_priority_order_with_mixed_duplicates():
    ...
    onedrive_read_file = _named(
        "tool_read_file", "th2agent.tools_store.portfolio.onedrive_read"
    )
    onedrive_list = _named("tool_list_files", "th2agent.tools_store.portfolio.onedrive_read")
    basic_read_file = _named("tool_read_file", "th2agent.tools_store.portfolio.basic")
    basic_pdf = _named("tool_pdf_to_images", "th2agent.tools_store.portfolio.basic")
    notify = _named("notify_user", "th2agent.core.agent_helpers.agent_utils")
