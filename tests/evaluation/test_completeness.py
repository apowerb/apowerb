...
async def test_refuses_to_judge_a_model_with_itself():
    ...
    with patch(f"{_SHARED}.get_settings") as mock_settings:
        mock_settings.return_value = MagicMock(
            ...
            evaluation_judge_api_key="k", evaluation_pass_threshold_completeness=0.7,
        )
...
async def test_missing_judge_config_raises_before_any_query():
    ...
    with patch(f"{_SHARED}.get_settings") as mock_settings:
        mock_settings.return_value = MagicMock(
            evaluation_judge_model="", evaluation_judge_api_key="", evaluation_pass_threshold_completeness=0.7
        )
...
async def test_details_carries_ordered_criteria():
    ...
    reply = MagicMock()
    reply.choices = [
        MagicMock(message=MagicMock(content='{"completeness": 0.4, "rationale": "ok"}'))
    ]
    ...
    assert outcome.details["criteria"] == [
        {"name": "completeness", "value": 0.4, "kind": "score"},
...
async def test_locale_fr_asks_the_judge_for_a_french_rationale():
    ...
    reply = MagicMock()
    reply.choices = [
        MagicMock(message=MagicMock(content='{"completeness": 1.0, "rationale": "Complet."}'))
    ]
